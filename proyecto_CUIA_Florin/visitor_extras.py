"""visitor_extras.py – *Solo* SouvenirMaker (sin estadísticas).
    ...
    Generación de souvenirs a partir de la ROI del frame actual.
    Este módulo se encarga de crear un souvenir (imagen) del frame actual
    con un fondo especial (avión) y una superposición de la cara del usuario
    si está disponible. Utiliza MediaPipe para segmentar la cara y
    guarda el souvenir.
```"""
